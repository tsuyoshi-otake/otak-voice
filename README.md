...
- **自動テスト**: Jestフレームワークを使用した46テストスイート・650テストにより、機能の安定性を確保
- **静的解析**: ESLint（Flat Config）による継続的なコード品質チェック（エラー0件）
- **セキュリティスキャン**: Snykによる継続的なセキュリティ脆弱性チェックを実施
- **コード品質**: 全ファイル298行以下のモジュール化アーキテクチャと明確な責任分離
...
4. 必要に応じて校閲ボタンをクリックし、gpt-5.2で内容を校正できます
...
- **自動校正**: ONにすると、音声認識結果をgpt-4.1-miniで自動的に校正します
...
### プロンプト設定
- **自動校正プロンプト**: gpt-4.1-miniによる自動校正時に使用するプロンプトをカスタマイズできます
- **校閲プロンプト**: gpt-5.2による校閲時に使用するプロンプトをカスタマイズできます

## 特殊機能の使い方

### 校閲機能
1. 校閲したいテキストが入力されている状態で校閲ボタン(✓)をクリックします
2. gpt-5.2による校閲が開始され、結果が入力フィールドに反映されます
...
npm run test:site-handlers     # サイトハンドラーテストのみ実行
